...
import { setDate, setLocation, logoutShCart } from '../../redux/shCartSlice'
            ...
            .then(response => {
                ...
                Object.keys(cart.orders).forEach(key => {
                    ...
                    .then(() => {
                        ...
                        navigation(`/ShoppingCartView?uuid=${cartUUID}`)  
                    })
...
export function ShoppingCartView(){
    ...
    return(
        ...
        <div className="shCart-container">
            ...
            <div className="shCart-view-label">{`Место доставки: ${cart.address}`}</div>
            <ShCartViewList token={user.token} uuid={params.get('uuid')}/>
        </div> : null
)}
