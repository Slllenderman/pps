...
import { setShCartDate, setShCartLocation } from '../../redux'
...
function ShoppingCart(){
    ...
    useEffect(() => {if(!store.isAuthorized) navigation("/") }, [])
    ...
    return(
        <div className="shCart-container">
            ...
            <input className="shCart-textBlock" type="text" defaultValue={store.shCartDate}
            onChange={(e) => dispatch(setShCartDate(e.target.value))}/>
            <div className="shCart-label">Место доставки</div>
            <div className="shCart-grid">
                <input className="shCart-textBlock" type="text" defaultValue={store.shCartLocation}
                onChange={(e) => dispatch(setShCartLocation(e.target.value))}/>
